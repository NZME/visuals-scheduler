#!/usr/bin/env python3
"""
Visuals Sunday Draft — On-Demand Sunday Snapshot
==================================================
Posts the standard single-day draft (shift times, jobs, edits — no
editorial notes section) for the upcoming Sunday from TeamUp and
Humanity to the Slack drafts channel.

"Upcoming Sunday" means today if today is Sunday, otherwise the next
Sunday. This is intended for /visuals-update sunday — typically run
over the weekend (or early Sunday) to refresh Sunday's job list if it
has changed since Friday's combined Sat/Sun/Mon draft went out.

Usage:
  python3 visuals_sunday_draft.py

Or trigger via GitHub Actions (repository_dispatch: sunday-draft-trigger),
fired by cron-job.org polling api/trigger.js for the pending_sunday_draft
Redis flag set by /visuals-update sunday.
...
def format_day_header(d):
    """Format a date as a bold Slack day header, e.g. '*Sunday 15 June*'."""
...
def get_upcoming_sunday(today):
    """
    Return the upcoming Sunday: today itself if today is Sunday,
    otherwise the next Sunday.
    """
    days_until_sunday = (6 - today.weekday()) % 7  # 0 if today is Sunday
...
def build_message(sunday):
    """Build the Sunday draft message — standard single-day format,
    ...
    # Header
    lines.append(format_day_header(sunday))
    lines.append("")

    # ── Visuals jobs ──────────────────────────────────────────────────────────
    all_events = get_events(sunday, TEAMUP_VISUALS_ID)
    ...
    for name in sorted(SHIFT_TIME_MEMBERS, key=lambda n: shift_sort_key(shifts, n, sunday)):
        if (name, sunday) not in shifts:
            continue  # not in CSV — skip rather than show _(time)_
        display = NAME_TO_SLACK.get(name, name)
        lines.append(f"{display} {shift_display(shifts, name, sunday)}")
    lines.append("")
    ...
    # ── Studio ─────────────────────────────────────────────────────────────────
    studio = get_events(sunday, TEAMUP_STUDIO_ID)
    ...
    # ── Edits ──────────────────────────────────────────────────────────────────
    edits = get_events(sunday, TEAMUP_EDITING_ID)
...
def main():
    ...
    sunday = get_upcoming_sunday(today)
    print(f"Fetching jobs for {sunday.strftime('%A %-d %B')} (today is {today.strftime('%A %-d %B')})...")
